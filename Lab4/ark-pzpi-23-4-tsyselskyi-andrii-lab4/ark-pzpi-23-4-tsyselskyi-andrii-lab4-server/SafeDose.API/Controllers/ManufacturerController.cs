...
namespace SafeDose.API.Controllers
{
    ...
    public class ManufacturerController : ControllerBase
    {
        ...
        public ManufacturerController(IApplicationDbContext db)
        {
            _db = db;
        }
        ...
        {
            var list = await _db.Manufacturers.ToListAsync();
            return Ok(list.Select(m => new { m.Id, m.Name }));
        }

        [HttpGet("{id}")]
        public async Task<IActionResult> Get(long id)
        {
            var entity = await _db.Manufacturers.FindAsync(id);
            if (entity == null)
                return NotFound();
        ...
        public async Task<IActionResult> Create([FromBody] CreateNameRequest dto)
        {
            try
            {
                var entity = new Manufacturer { Name = dto.Name };
